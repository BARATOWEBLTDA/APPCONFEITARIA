...
export function PerfilTab({ accent, confeteiraUserId }: { accent: string; confeteiraUserId: string }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setErro('Telefone não encontrado. Faça seu cadastro!')
      }
  ...
  const handleCadastro = async () => {
    ...
    try {
      ...
      const { data: existe } = await supabase
        ...
        .maybeSingle()
      if (existe) {
        const c = { id: existe.id, nome: existe.nome, telefone: existe.telefone }
        setCliente(c)
        localStorage.setItem(`cardapio_cliente_${confeteiraUserId}`, JSON.stringify(c))
        buscarPedidos(existe.telefone)
        setShowCadastro(false)
      } else {
        const { data: novo } = await supabase
          .from('clientes')
          .insert({ user_id: confeteiraUserId, nome: cadNome.trim(), telefone: cadTel, email: cadEmail || null })
          .select('id, nome, telefone')
          .single()
        if (novo) {
          const c = { id: novo.id, nome: novo.nome, telefone: novo.telefone }
          setCliente(c)
          localStorage.setItem(`cardapio_cliente_${confeteiraUserId}`, JSON.stringify(c))
          buscarPedidos(novo.telefone)
          setShowCadastro(false)
        }
      }
  ...
  const inp = (placeholder: string, value: string, onChange: (v:string)=>void, tel?: boolean) => (
    <div style={{display:'flex',alignItems:'center',gap:'10px',padding:'14px',border:'1.5px solid #f0f0f0',borderRadius:'12px',background:'#fafafa'}}>
      <input
        value={value}
        onChange={e => onChange(tel ? maskTel(e.target.value) : e.target.value)}
      ...
      {showLogin && (
        <>
          ...
          <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:301,background:'#fff',borderRadius:'24px 24px 0 0',padding:'24px 20px 32px',maxWidth:'480px',margin:'0 auto'}}>
            ...
            <p style={{margin:'0 0 20px',fontSize:'13px',color:'#a0a0a0',textAlign:'center',lineHeight:'1.5'}}>Informe seu telefone para continuar ou criar uma nova conta.</p>
            ...
            <p style={{margin:'16px 0 0',textAlign:'center',fontSize:'13px',color:'#a0a0a0'}}>
              ...
              <button onClick={() => { setShowLogin(false); setShowCadastro(true); setErro('') }}
                style={{background:'none',border:'none',color:accent,fontWeight:700,cursor:'pointer',fontSize:'13px',fontFamily:'inherit'}}>
                Criar cadastro
              </button>
            </p>
          </div>
      ...
      {showCadastro && (
        <>
          ...
          <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:301,background:'#fff',borderRadius:'24px 24px 0 0',padding:'24px 20px 32px',maxWidth:'480px',margin:'0 auto'}}>
            ...
            <p style={{margin:'0 0 20px',fontSize:'13px',color:'#a0a0a0',textAlign:'center',lineHeight:'1.5'}}>Preencha seus dados para criar uma conta e fazer seu pedido.</p>
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
        ...
        {aba === 'dados' && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            ...
            <button onClick={handleSair}
              style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',padding:'14px',background:'none',border:'1.5px solid #fee2e2',borderRadius:'12px',color:'#dc2626',fontWeight:600,fontSize:'14px',cursor:'pointer',fontFamily:'inherit',marginTop:'4px'}}>
