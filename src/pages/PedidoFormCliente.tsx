...
// ── Gera horários de 07:00 a 22:00 de 30 em 30 min ──────────────────────────
...
function HorarioSheet({ value, onChange, onClose }: {
  ...
  // Scroll até o horário selecionado
  useEffect(() => {
            ...
            <div className="hs-list" ref={listRef}>
              {HORARIOS.map(hora => (
                <button
                  key={hora}
                  data-hora={hora}
                  className={`hs-item${value === hora ? ' hs-item--on' : ''}`}
    ...
    <div className="step-root">
      ...
      <div className="pf2-card">
        ...
        <div className="pf2-row">
          ...
          {/* Horário — bottom sheet no mobile, TimePickerField no desktop */}
          <div className="pf2-field">
            <label className="pf2-label">Horário</label>
            {isMobile ? (
              <button
                type="button"
                className={`pf2-input pf2-hora-btn${pedido.horario_entrega ? ' pf2-hora-btn--filled' : ''}`}
                onClick={() => setShowHorarioSheet(true)}
              >
      ...
      {showHorarioSheet && (
        <HorarioSheet
          ...
          onClose={() => setShowHorarioSheet(false)}
        />
      )}

      <style>{`
        /* ── Campo botão de horário ── */
        .pf2-hora-btn {
        ...
        .pf2-native-display input[type="date"] {
          ...
          width: 100%; height: 100%; cursor: pointer; font-size: 16px;
        }

        /* ── Bottom Sheet ── */
        .hs-overlay {
          position: fixed; inset: 0; background: rgba(0,0,0,0.45);
          z-index: 100; animation: hsFadeIn 0.2s ease;
        }
        .hs-sheet {
          position: fixed; bottom: 0; left: 0; right: 0;
          background: var(--bg-card, #fff);
          border-radius: 20px 20px 0 0;
          z-index: 101; max-height: 72vh;
        ...
        .hs-item {
          ...
          font-size: 1rem; font-family: 'Geist', sans-serif;
          font-weight: 500; color: var(--text-primary, #431524);
          cursor: pointer; transition: background 0.1s;
          border-bottom: 0.5px solid var(--border, #ECC2D0);
        }
        ...
        .hs-custom-input {
          ...
          font-size: 1.5rem; font-family: 'Geist', sans-serif;
          color: var(--text-primary, #431524);
          text-align: center; letter-spacing: 0.1em;
          background: var(--bg-input, #fff); outline: none; width: 100%;
        }
        ...
        .hs-btn-primary {
          flex: 2; padding: 12px;
          background: var(--primary, #986274); color: white;
